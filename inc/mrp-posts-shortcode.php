...
function MRP_Rest_Post_Shortcode( $atts ) {
		...
		$mrp_post_id =	'restpost_transient_id_'.$a['id'];

	// Get any existing copy of our transient data
	if ( false === ( $cached_response = get_transient( $mrp_post_id ) ) ) {
	    // It wasn't there, so regenerate the data and save the transient
			// Start REST API
			if (empty($a['offset'])) {
		 		$cached_response = wp_remote_get( $a['url'] . '/wp-json/wp/v2/'.$a['post_type'].'?per_page=' . $a['count'] );
		 	}
		 	else {
		 		$cached_response = wp_remote_get( $a['url'] . '/wp-json/wp/v2/'.$a['post_type'].'?per_page=' . $a['count'] . '&offset=' . $a['offset'] );
		 	}
		 	if( is_wp_error( $cached_response ) ) {
		 		return;
		 	}
	     set_transient( $mrp_post_id, $cached_response, 12 * HOUR_IN_SECONDS );
	}


	$posts = json_decode( wp_remote_retrieve_body( $cached_response ) );
